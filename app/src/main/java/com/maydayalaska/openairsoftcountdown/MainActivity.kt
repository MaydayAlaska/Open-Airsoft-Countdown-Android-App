...
class MainActivity : Activity()
{
	private companion object
	{
		...
		const val PayPalDonationUrl = "https://paypal.me/lorisgennarini"
	...
	private val commitDurationSelection = Runnable { commitSelectedDuration() }
	...
	private var isDurationPickerScrolling = false
	private var hasPendingDurationSelection = false
	...
	{
		handler.removeCallbacks(commitDurationSelection)
		...
		isDurationPickerScrolling = false
		hasPendingDurationSelection = false
	...
	private fun renderMainScreen()
	{
		...
		val durationPickerRow = LinearLayout(this)
	...
	private fun createDurationPicker(maxValue: Int, description: String): NumberPicker
	{
		...
		{
			(picker.getChildAt(index) as? EditText)?.apply {
				textSize = 36f
		...
		picker.setOnValueChangedListener { _, _, _ ->
			...
			{
				hasPendingDurationSelection = true
				scheduleDurationCommit()
			}
		}
		picker.setOnScrollListener { _, scrollState ->
			when (scrollState)
			{
				...
				{
					isDurationPickerScrolling = false
					commitSelectedDuration()
				}
				else ->
				{
					isDurationPickerScrolling = true
					handler.removeCallbacks(commitDurationSelection)
				}
			}
		}
	...
	private fun showStartupDialog()
	{
		...
		val welcomeMessage = TextView(this)
		welcomeMessage.text = tr(
			"Benvenuto in Open Airsoft Countdown!\n\n" +
				"Grazie per aver scelto questo progetto open source. " +
				"Se vuoi sostenerlo, metti una Star su GitHub e, su MakerWorld, " +
				"lascia un Like, salva il progetto e usa un Boost. " +
				"Puoi anche contribuire allo sviluppo con una donazione tramite PayPal.\n\n" +
				"Per segnalare bug o problemi, usa la sezione Issues nelle repository GitHub del progetto.",
			"Welcome to Open Airsoft Countdown!\n\n" +
				"Thank you for choosing this open-source project. " +
				"To support it, give it a Star on GitHub and, on MakerWorld, " +
				"leave a Like, save the project, and use a Boost. " +
				"You can also support development with a donation through PayPal.\n\n" +
				"To report bugs or other issues, please use the Issues section in the project's GitHub repositories."
		)
		welcomeMessage.textSize = 15f
		welcomeMessage.setTextColor(palette.textPrimary)
		welcomeMessage.setLineSpacing(0f, 1.08f)
		dialogContent.addView(welcomeMessage, matchWrap())

		val firmwareGithubButton = createActionButton(
			tr("Repository firmware su GitHub", "Firmware repository on GitHub"),
			ButtonStyle.Secondary,
			false
		) {
			openUrl(FirmwareRepositoryUrl)
		}
		dialogContent.addView(firmwareGithubButton, matchWrapWithTopMargin(16))

		val androidGithubButton = createActionButton(
			tr("Repository app Android su GitHub", "Android app repository on GitHub"),
			ButtonStyle.Secondary,
			false
		) {
			openUrl(AndroidRepositoryUrl)
		}
		dialogContent.addView(androidGithubButton, matchWrapWithTopMargin(8))

		val makerWorldButton = createActionButton(
			tr("Apri MakerWorld", "Open MakerWorld"),
			ButtonStyle.Secondary,
			false
		) {
			openUrl(MakerWorldProfileUrl)
		}
		dialogContent.addView(makerWorldButton, matchWrapWithTopMargin(8))

		val paypalButton = createActionButton(
			tr("Dona con PayPal", "Donate with PayPal"),
			ButtonStyle.Primary,
			false
		) {
			openUrl(PayPalDonationUrl)
		}
		dialogContent.addView(paypalButton, matchWrapWithTopMargin(8))

		AlertDialog.Builder(this)
			.setTitle(tr("Benvenuto!", "Welcome!"))
	...
	private fun scheduleDurationCommit()
	{
		if (!isDurationPickerEditable() || isDurationPickerScrolling)
		{
			return
		}

		handler.removeCallbacks(commitDurationSelection)
		handler.postDelayed(commitDurationSelection, 350)
	}

	private fun commitSelectedDuration()
	{
		handler.removeCallbacks(commitDurationSelection)

		if (!hasPendingDurationSelection || !isDurationPickerEditable() || isDurationPickerScrolling)
		{
			return
		}
	...
	private fun updateTimerControls()
	{
		...
		startPauseButton?.isEnabled = timerControlReady
		startPauseButton?.alpha = if (timerControlReady) 1f else 0.45f
		...
		durationPickerHintLabel?.text = when
		{
			...
			cachedMode.equals("SET_TIMER", ignoreCase = true) -> tr("Scorri ore, minuti o secondi per impostare la durata.", "Scroll hours, minutes, or seconds to set the duration.")
			cachedMode.equals("STOPPED", ignoreCase = true) -> tr("Timer in pausa. Premi Stop per ripristinare la durata.", "Timer paused. Press Stop to restore the duration.")
			cachedMode.equals("RUNNING", ignoreCase = true) -> tr("Timer in esecuzione.", "Timer is running.")
			else -> tr("Premi Stop per ripristinare e modificare la durata.", "Press Stop to restore and change the duration.")
		}
	}
